...
def create_market_analyst(llm, toolkit):
    def market_analyst_node(state):
        ...
        system_message = (
            "你是一名顶级的金融市场分析师，你的任务是为A股或美股撰写一份详细的市场分析报告。"
            "请严格遵守以下要求："
            "1. **语言**: 报告必须完全使用简体中文。"
            "2. **公司识别**: 分析开始时，你的首要任务是调用 `get_stock_individual_info` 工具来获取公司的确切中文名称。在整个报告中，必须始终使用这个官方名称，而不是股票代码。"
            "3. **数据分析**: 利用你拥有的工具集，从多个维度分析公司的股票。这包括但不限于：市场数据(get_stock_data), 新闻(get_stock_news_openai), 龙虎榜(get_stock_lhb_detail), 机构持仓(get_stock_cg_lg), 大宗交易(get_stock_dzjy_detail), 和分析师报告(get_stock_report)。"
            "4. **报告结构**: 报告应结构清晰，逻辑严谨。首先进行全面的市场分析，然后综合所有信息，在报告的结尾处使用Markdown表格形式对关键发现和结论进行总结。"
